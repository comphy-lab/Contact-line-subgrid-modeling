...
an explicit coefficient for arbitrary $(\theta_e,\mur)$ and includes the
closed-form right-angle result; a general-angle evaluator would additionally
require solving Hocking's integral equation and is deliberately not guessed.
...
\]
It also gives the finite-viscosity-ratio values $c(\pi/2,1)=2.05$ and
$c(\pi/2,10)=6.40$ reported by Chan \emph{et al.}~\cite{chan2020}. Values away
from $\theta_e=\pi/2$ remain an explicit input rather than an undocumented
approximation.
